    ...
    <main class="quick-main">
      ...
      <section>
        <div class="wrap">
          ...
          <div
            v-else-if="accountsLoading || catalogLoading"
            class="state"
            v-reveal
          >
            正在加载快捷导入数据…
            ...
            <div class="wiz-card" v-reveal>
              ...
              <div class="op-search">
                ...
                <span class="op-search-count"
                  >本页 {{ pageOperators.length }} 位 · 已有数据
                  {{ hasDataOperators.length }} 位 · 目录
                  {{ catalogCount }} 位</span
                >
              </div>
              ...
              <template v-if="operatorGroups.length">
                <template v-for="(g, gi) in operatorGroups" :key="gi">
                  <div v-if="g.divider" class="op-divider">
                    <span>{{ g.label }}</span>
                  </div>
...
const starSteps = [
  {
    ...
    nav: "1星",
    title: "选择 1 星密探",
    sub: "勾选当前为「1 星」的密探（节点 0~5）。",
  },
  {
    key: "2",
    nav: "2星",
    title: "勾选 2 星密探",
    sub: "勾选当前为「2 星」的密探（节点 0~5）。",
  },
  {
    key: "3",
    nav: "3星",
    title: "勾选 3 星密探",
    sub: "勾选当前为「3 星」的密探（节点 0~5）。",
  },
  {
    key: "4",
    nav: "4星",
    title: "勾选 4 星密探",
    sub: "勾选当前为「4 星」的密探（节点 0~5）。",
  },
  {
    key: "5",
    nav: "5星",
    title: "勾选 5 星密探",
    sub: "勾选当前为「5 星」的密探（节点 0~5）。",
  },
  {
    key: "awaken",
    nav: "觉醒",
    title: "勾选 觉醒 密探",
    sub: "勾选当前已「觉醒」的密探（仅一档）。",
  },
];
...
const currentKey = computed(function () {
  return currentStep.value.key;
});
const isAwaken = computed(function () {
  return currentKey.value === "awaken";
});
const checkedOfCurrent = computed(function () {
...
const pageOperators = computed(function () {
  ...
  return catalogOperators.value
    ...
    .filter(function (op) {
      if (!q) return true;
      const hay = [op.name, op.alias, op.id, op.prof, op.subProf]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
...
const operatorGroups = computed(function () {
  const groups = [];
  if (noDataOperators.value.length)
    groups.push({ divider: false, items: noDataOperators.value });
  if (hasDataOperators.value.length)
    groups.push({
      divider: true,
...
function stepLabel(key) {
  const step = starSteps.find(function (s) {
    return s.key === key;
  });
...
function removeFromOtherSteps(ids, keepKey) {
  ...
  starSteps.forEach(function (s) {
    ...
    checkedByKey[s.key] = (checkedByKey[s.key] || []).filter(function (id) {
      return !idSet.has(id);
    });
  });
}
...
watch(
  function () {
    return currentKey.value && (formByKey[currentKey.value] || {}).level;
  },
  function (level) {
...
watch(
  function () {
    return currentKey.value && (formByKey[currentKey.value] || {}).elite;
  },
  function (elite) {
    const f = formByKey[currentKey.value];
    if (!f) return;
  ...
  return ids
    .map(function (id) {
      ...
      return {
        ...
        subProf: Array.isArray(op.subProf)
          ? op.subProf
          : op.subProf
            ? op.subProf.split("、")
            : [],
...
async function loadAccounts() {
  ...
  try {
    ...
    const candidate =
      queryId &&
      accounts.value.some(function (a) {
        return a.id === queryId;
      })
        ? queryId
...
.page-save {
  ...
  border: 1.5px solid rgba(215, 137, 53, 0.45);
  color: var(--ink);
}
.page-save.ok-static {
  background: var(--paper);
...
.page-save.err {
  ...
  border: 1.5px solid rgba(166, 81, 74, 0.4);
  color: var(--rouge);
}
.page-save .link {
  margin-left: 6px;
...
.op-card:hover {
  border-color: var(--accent);
  box-shadow: 0 10px 22px -14px rgba(73, 59, 44, 0.3);
}
.op-card.on {
  background: var(--yellow);
  border-color: var(--yellow-deep);

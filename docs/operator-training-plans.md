...
- 「特别关注」清单的成员为：最新星标集合 ∪ 手动添加集合 − 手动移出集合。移出清单不取消星标，不删除密探档案，不扣库存。管理密探可重新加入被移出的成员。
- 用户可删除任意培养清单，包括「特别关注」，并允许保存零清单工作区。零清单时点击添加，默认建议根据当前特别关注重建动态清单，同时允许改选空白自建清单。
...
- 移出密探、删除培养清单均提供一次撤销；删除清单需要二次确认。后续清单修改会替换/清除该撤销记录。撤销是当前页面内的交互，不是持久化回收站。
- 管理清单时以当前密探目录为准；重新保存会自动移除已不在目录中的成员 ID、排除 ID 和目标记录，防止 `Unknown operator` 导致整个 workspace 保存失败。
...
- 特别关注清单动态同步星标，排除/重新加入均有效；移出与撤销不改星标。自建计划同一密探不同目标互不覆盖。
- 任意清单可删除，全部删除后刷新仍保持空工作区；再次添加时默认建议根据特别关注建立。

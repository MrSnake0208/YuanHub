...
test('反馈未读状态已接入导航角标和管理详情', () => {
  ...
  assert.match(sidebar, /notificationUnreadState\.count/)
  assert.doesNotMatch(sidebar, /getUnreadNotificationCount/)
  assert.doesNotMatch(sidebar, /fetchUnreadCount|unreadPollTimer/)
...
test('用户反馈按通知 refId 显示逐工单未读标识并只清理当前工单', () => {
  ...
  assert.match(feedbackPage, /notificationUnreadState\.feedbackRefIds/)
  assert.match(feedbackPage, /subscribeNotificationUnread\(\{ details: true \}\)/)
  assert.match(feedbackPage, /refreshNotificationUnreadDetails\(\)/)
  assert.match(feedbackPage, /markFeedbackNotificationsRead\(reportId, unreadNotifications\)/)
  assert.match(feedbackPage, /removeNotificationUnreadItems\(marked\.map/)
  assert.doesNotMatch(feedbackPage, /listUnreadNotifications|getUnreadFeedbackNotificationRefs/)

...
test('分享页概览使用 v3 只读卡片，不提供点击详情弹窗', () => {
  const card = sharePage.match(/<article[\s\S]*?<\/article>/)
  assert.ok(card)
  ...
  assert.doesNotMatch(card[0], /class="rarity"|ledger-step-actions|ledger-smart-action|ledger-next-action/)
  ...
  assert.match(card[0], /<span v-if="!entry\.sp_of">\{\{ starCardNode\(entry\.growth\.star_level\) \}\}<\/span>/)
  assert.doesNotMatch(card[0], /节点|operatorShareStarCompleteLabel|share-growth-complete/)
  ...
  assert.match(sharePage, /v-model:search-query="searchQuery"/)
  assert.match(sharePage, /v-model:prof-filter="profFilter"/)
  assert.match(sharePage, /v-model:sub-prof-filter="subProfFilter"/)
  assert.match(sharePage, /v-model:status-filter="statusFilter"/)
  assert.match(sharePage, /filterOperatorShareEntries\(entries\.value, searchQuery\.value, profFilter\.value, subProfFilter\.value, statusFilter\.value\)/)
  assert.match(sharePage, /OperatorFilterDossier/)
  assert.match(operatorPage, /OperatorFilterDossier/)
  assert.match(filterDossier, /<strong>\{\{ title \}\}<\/strong>/)
  assert.match(filterDossier, /\.pf-label \{[\s\S]*?border: 0;[\s\S]*?background: transparent;/)
  ...
  assert.match(ledgerVersionConfig, /ACTIVE_OPERATOR_LEDGER_CARD_VERSION = OPERATOR_LEDGER_CARD_VERSIONS\.V3/)
  ...
  assert.match(ledgerV2Styles, /\.agent-ledger-card--share\.agent-ledger-card--v2 \.ledger-combat-stat \{[\s\S]*?grid-template-columns: minmax\(0, 1fr\) 14px max-content minmax\(0, 1fr\);[\s\S]*?column-gap: 8px;[\s\S]*?padding: 4px 2px 8px 0px;/)
  ...
  assert.match(ledgerV2Styles, /@media \(max-width: 640px\)[\s\S]*?\.agent-ledger-card--share\.agent-ledger-card--v2 \.ledger-combat-stat \{\s*padding-bottom: 11px;/)
  assert.match(ledgerV2Styles, /\.agent-ledger-card--share\.agent-ledger-card--v2 \.share-ledger-growth \{[\s\S]*?grid-template-columns: repeat\(3, minmax\(0, 1fr\)\);[\s\S]*?row-gap: 0;/)
  assert.match(ledgerV2Styles, /\.share-ledger-growth \.ledger-growth-row:nth-child\(3\) \{[\s\S]*?grid-column: auto;[\s\S]*?grid-template-columns: auto auto;[\s\S]*?width: max-content;/)
  assert.match(ledgerV2Styles, /\.share-ledger-growth \.ledger-growth-row:nth-child\(1\) \{\s*justify-self: start;/)
  assert.match(ledgerV2Styles, /\.share-ledger-growth \.ledger-growth-row:nth-child\(2\) \{\s*justify-self: center;/)
  assert.match(ledgerV2Styles, /\.share-ledger-growth \.ledger-growth-row:nth-child\(3\) \{\s*justify-self: end;/)
  assert.match(ledgerV2Styles, /\.agent-ledger-card--share\.agent-ledger-card--v2 \.share-ledger-growth-value \{[\s\S]*?display: inline-flex;[\s\S]*?white-space: nowrap;/)
  assert.match(ledgerV2Styles, /\.share-ledger-growth-value\.ledger-huaji-value \{[\s\S]*?justify-content: flex-start;[\s\S]*?overflow: visible;/)
  assert.match(ledgerV2Styles, /\.share-ledger-growth-value\.ledger-huaji-value > span,[\s\S]*?\.share-ledger-growth-value\.ledger-huaji-value > svg \{[\s\S]*?transform: none;/)

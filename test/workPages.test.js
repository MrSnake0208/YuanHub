...
test('详情页覆盖错误、不可转换、兼容性和安全文本边界', function () {
  ...
  const compatibility = readSource('../src/components/work/WorkCompatibilityResult.vue')
  assert.match(page, /w_\[0-9a-fA-F\]\{24\}/)
...
test('结构化编辑器保留 WorkDocument 顺序、显式保存、兼容性预览和冲突边界', function () {
  const page = readSource('../src/pages/work/editor.vue')
  const issueList = readSource('../src/components/work/IssueList.vue')
  const compatibility = readSource('../src/components/work/WorkCompatibilityResult.vue')
  assert.match(page, /v-for="slot in 5"/)
  assert.match(page, /moveAction/)
  assert.match(page, /上移/)
  assert.match(page, /下移/)
  assert.match(page, /createWork/)
  assert.match(page, /getOwnedWork/)
  assert.match(page, /hydrateWorkDocument/)
  assert.match(page, /previewWorkCompatibility/)
  assert.match(page, /publishWork/)
  assert.match(page, /expectedRevision|revision\.value/)
  assert.match(page, /error\?\.status === 409/)
  assert.match(page, /reloadLatest/)
  assert.match(page, /beforeunload/)
  assert.match(page, /onBeforeRouteLeave/)
  assert.match(page, /onBeforeRouteUpdate/)
  assert.match(page, /请先保存当前修改/)
  assert.match(page, /id="work-validation-summary"/)
  assert.match(page, /aria-describedby/)
  assert.match(page, /aria-invalid/)
  assert.doesNotMatch(page, /draggable|autosave|setInterval/)
  assert.match(issueList, /issue\.path/)
  assert.match(compatibility, /target_document/)
  assert.match(compatibility, /navigator\.clipboard/)
})

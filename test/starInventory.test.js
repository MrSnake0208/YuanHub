...
test('星石页以 YuanHub 壳层挂载已构建的 YuanStar 产品', function () {
  ...
  assert.match(page, /数据交换/)
  assert.doesNotMatch(page, /同步背包/)
  assert.match(page, /Archive/)
  assert.match(page, /\.archive-toggle\s*\{[^}]*border:\s*1\.5px solid var\(--line\)/)
  assert.match(page, /ArchiveExchangePanel/)
  assert.match(page, /showArchive \? "收起数据交换" : "数据交换"/)
  assert.match(page, /handle\.exportDataExchange\(\)/)
  assert.match(page, /handle\.previewDataExchangeImport\(file\)/)
  assert.match(page, /handle\.confirmDataExchangeImport\(preview\)/)
  assert.match(page, /accept="\.json,application\/json"/)
  assert.doesNotMatch(page, /\.xlsx|XLSX/)
  assert.match(page, /:disabled="!auth\.isLoggedIn \|\| accountsLoading \|\| accountBusy \|\| starExchangeBusy"/)
  assert.match(page, /:game-disabled="!auth\.isLoggedIn \|\| accountsLoading \|\| accountBusy \|\| starExchangeBusy"/)
  assert.match(page, /function resetStarImportState\(\)/)
  assert.match(page, /bindStarExchangePreview\(preview, previewAccountId\)/)
  assert.match(page, /isStarExchangePreviewCurrent\(starImportPreview\.value, accountId\.value\)/)
  assert.match(page, /账号已切换，请重新选择 JSON 档案后再试。/)
  assert.doesNotMatch(page, /handle\.openDataTransfer\(\)/)
  assert.match(page, /onReplacementImport:\s*function\s*\(\s*snapshot\s*\)\s*\{\s*return starCloud\.replaceImport\(snapshot\)/)
  assert.doesNotMatch(page, /syncCurrentStarInventory/)

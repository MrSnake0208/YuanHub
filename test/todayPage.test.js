...
test('uses 今日一览 as the default page', function () {
  ...
  assert.match(page, /还没有可用于今日一览的数据/)
  assert.match(page, /MaaYuan 自动同步/)
  assert.match(page, /推荐/)
  assert.match(page, /同步至YuanHub/)
  assert.match(page, /YuanHub连接码/)
  assert.match(page, /to="\/user\/profile#maayuan-app-title"/)
  assert.match(page, /manualEntryTo/)

...
test('密探主观数据与快捷提升 API 使用后端约定的路径、字段和幂等头', async function () {
  ...
  try {
    ...
    await previewOperatorUpgrade({ accountId: 'acc one', game: '如鸢', operatorId: 'char/1', dimension: 'huaji', target: 22, expectedOperatorRevision: 7 })
    await executeOperatorUpgrade({ accountId: 'acc one', game: '如鸢', operatorId: 'char/1', dimension: 'huaji', target: 22, expectedOperatorRevision: 7, expectedInventoryRevision: 42, previewToken: 'token', idempotencyKey: 'uuid' })
  ...
  assert.deepEqual(JSON.parse(calls[4].options.body), {
    account_id: 'acc one', game: '如鸢', operator_id: 'char/1', dimension: 'huaji', target: 22, expected_operator_revision: 7
  })
  assert.equal(calls[5].options.headers['Idempotency-Key'], 'uuid')
  assert.deepEqual(JSON.parse(calls[5].options.body), {
    account_id: 'acc one', game: '如鸢', operator_id: 'char/1', dimension: 'huaji', target: 22,

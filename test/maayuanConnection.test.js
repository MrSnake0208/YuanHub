...
test('今日一览可以直接跳到连接码页面并自动展开 MaaYuan 连接面板', () => {
  assert.match(profile, /useRoute/)
  assert.match(profile, /route\.query\.connect !== "maayuan"/)
  assert.match(profile, /async function openMaaYuanFromRoute/)
  assert.match(profile, /showMaaYuanConnect\.value = true/)
  assert.match(profile, /id="maayuan-connect-panel"/)
  assert.match(profile, /ref="maaYuanAppCard"/)
  assert.match(profile, /ref="maaYuanAppTitle"/)
  const start = profile.indexOf('async function openMaaYuanFromRoute')
  const end = profile.indexOf('function showCreatedToken', start)
  const routeOpen = profile.slice(start, end)
  assert.match(routeOpen, /maaYuanAppCard\.value/)
  assert.match(routeOpen, /appCard\.scrollIntoView/)
  assert.match(routeOpen, /maaYuanAppTitle\.value\.focus\(\{ preventScroll: true \}\)/)
  assert.doesNotMatch(routeOpen, /panel\.scrollIntoView/)

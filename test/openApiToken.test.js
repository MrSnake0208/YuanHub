...
test('hasEveryScope 判断现有 Token 是否可供 MaaYuan 使用', () => {
  assert.equal(hasEveryScope(['inventory:write', 'operator:scan:write', 'operator:read', 'star:capture:write'], MAAYUAN_REQUIRED_SCOPES), true)
  assert.equal(hasEveryScope(['inventory:write', 'operator:scan:write', 'star:capture:write'], MAAYUAN_REQUIRED_SCOPES), false)
...
test('mergeScopes 补全 MaaYuan 权限时保留已有权限并去重', () => {
  assert.deepEqual(
    ...
    ['inventory:read', 'inventory:write', 'operator:scan:write', 'operator:read', 'star:capture:write']
  )
})
